...
class UserFeedback(BaseModel):
    ...
    def validate_rating(cls, v, values):
        """Rating validation based on feedback type."""
        label = values.get("label")
        if label in [FeedbackLabel.LIKE, FeedbackLabel.DISLIKE] and v is not None:
            raise ValueError("Rating should not be provided for like/dislike feedback")
        return v
...
class FeedbackRequest(BaseModel):
    ...
    question: Optional[str] = Field(None, description="Original question (optional, can be retrieved by message_id)")
    model_answer: Optional[str] = Field(None, description="Answer from RAG (optional, can be retrieved by message_id)")
    user_feedback: UserFeedback
    
    # Anti-spam fields
    user_agent: Optional[str] = Field(None, max_length=500)
    session_fingerprint: Optional[str] = Field(None, max_length=128)
...
class FeedbackResponse(BaseModel):
    ...
    status: str = Field(..., description="queued, applied, spam_filtered, content_filtered, or error")
...
class FeedbackStatsResponse(BaseModel):
    ...
    avg_rating: float
    statistics: Optional[Dict[str, Any]] = None  # Дополнительные данные для фронтенда