"""API endpoints for feedback system - Fixed version."""
...
def _normalize_context_item(x: Any) -> Dict[str, Any]:
    """Приводит элемент контекста к виду для ContextInfo схемы."""
    if isinstance(x, dict):
        # Извлекаем текст контента
        content = x.get("text") or x.get("content") or ""
        
        # Извлекаем обязательные поля
        doc_id = x.get("doc_id") or x.get("document_id") or 0
        chunk_id = x.get("chunk_id") or x.get("id") or 0
        score = x.get("score") or 0.0
        
        # Извлекаем метаданные
        metadata = x.get("metadata") or {}
        if not metadata:
            metadata = {k: v for k, v in x.items() 
                       if k not in ("text", "content", "doc_id", "chunk_id", "score", "document_id", "id")}
    ...
    # объект с атрибутами
    content = getattr(x, "text", None) or getattr(x, "content", None) or ""
    doc_id = getattr(x, "doc_id", None) or getattr(x, "document_id", None) or 0
    chunk_id = getattr(x, "chunk_id", None) or getattr(x, "id", None) or 0
    score = getattr(x, "score", None) or 0.0
    metadata = getattr(x, "metadata", None) or {}
    
    if not metadata:
        metadata = {}
        for k in ("title", "source", "path", "uri"):
            v = getattr(x, k, None)
            if v is not None:
                metadata[k] = v
...
async def submit_feedback(
    ...
    """
    Submit user feedback for a response with advanced learning capabilities.
    ...
    try:
        ...
        # Process feedback through enhanced handler (ИСПРАВЛЕНО: убрал user_ip)
...
async def feedback_health():
    ...
    try:
        ...
        return {
            ...
            "learning_engine_available": hasattr(feedback_handler, 'learning_engine'),
            "trust_scorer_available": hasattr(feedback_handler, 'trust_scorer'),
            "content_filter_available": hasattr(feedback_handler, 'content_filter')
        }
    except Exception as e:
