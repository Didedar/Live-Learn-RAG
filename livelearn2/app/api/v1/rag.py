...
async def debug_retrieve(
    ...
    """
    Debug endpoint для диагностики ретривера.
    
    Показывает топ-k результатов с подробными скорами и превью чанков.
    """
    try:
        ...
        # Sanitize query
        clean_query = q.strip()[:500]  # Limit query length
        
        logger.info(f"Debug retrieve for query: {clean_query[:100]}...")
        
        # Get contexts with detailed scoring
        contexts = await rag_pipeline.retrieve_with_feedback(
            db=db,
            query=clean_query,
            top_k=k,
        ...
        return {
            ...
            "diagnostics": {
                ...
                "min_score": min([r["score"] for r in debug_results]) if debug_results else 0,
                "max_score": max([r["score"] for r in debug_results]) if debug_results else 0
            }
        }
...
@router.post("/ingest/egov-data")
async def ingest_egov_data(
    data: List[Dict[str, Any]],
    db: Session = Depends(get_db),
    user_id: Optional[str] = Depends(optional_auth)
):
    """
    Специальный endpoint для загрузки данных eGov услуг.
    
    Принимает массив записей с полями:
    - id: уникальный идентификатор услуги
    - name: название услуги  
    - eGov_link: ссылка на услугу на портале egov.kz (русская версия)
    - chunks: подробное описание услуги и процедур получения
    - eGov_kaz_link: ссылка на услугу на казахском языке
    """
    try:
        if not data or not isinstance(data, list):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Data must be a non-empty list of eGov service records"
            )
        
        logger.info(f"Processing {len(data)} eGov service records")
        ...
        for i, record in enumerate(data):
            ...
            for field in required_fields:
                if field not in record:
                    raise HTTPException(
                        status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                        detail=f"Record {i} missing required field: {field}"
                    )
        
        # Объединяем все записи в один документ для векторизации
        combined_text_parts = []
        
        for record in data:
            # Формируем структурированный текст для каждой услуги
            service_text = f"""
=== Государственная услуга №{record['id']}: {record['name']} ===

Название услуги: {record['name']}

Подробная информация:
{record['chunks']}

Ссылка на услугу (русский): {record.get('eGov_link', 'Не указана')}
Ссылка на услугу (казахский): {record.get('eGov_kaz_link', 'Не указана')}

---
"""
            combined_text_parts.append(service_text.strip())
        
        # Объединяем все услуги в один большой документ
        combined_text = "\n\n".join(combined_text_parts)
        
        # Подготавливаем метаданные
        metadata = {
            "source": "egov_services_batch",
            "total_services": len(data),
            "ingested_by": user_id or "anonymous",
            "data_type": "government_services",
            "language": "ru-kz",
            "service_ids": [str(record['id']) for record in data]
        }
        ...
        try:
            doc_id, chunk_count = await rag_pipeline.ingest_text(
                ...
                text=combined_text,
                metadata=metadata,
                uri=f"egov_services_batch_{len(data)}_records"
            )
            
            logger.info(
                f"Successfully ingested eGov data: {len(data)} services -> "
                f"document {doc_id} with {chunk_count} chunks"
            )
            
            return {
                "document_id": doc_id,
                "chunks": chunk_count,
                "services_processed": len(data),
                "status": "success"
            }
            
        except Exception as e:
            logger.error(f"Failed to ingest eGov data: {e}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to process eGov data: {str(e)}"
            )
    
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Unexpected error in eGov data ingestion: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An unexpected error occurred during eGov data processing"
        )